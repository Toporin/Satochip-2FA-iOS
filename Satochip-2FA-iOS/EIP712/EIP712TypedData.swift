...
//        print("regular")
    ...
    private func makeABIValue(name: String, data: JSON?, type: String) throws -> ABIValue {
        ...
        //} else if type == "bytes", let value = data?.stringValue, let valueData = value.hs.hexData {
        } else if type == "bytes", let value = data?.stringValue, let valueData = value.hexToBytes ?? nil {
            return try ABIValue(valueData.sha3, type: .bytes(32))
            ...
            if let length = Int(type.dropFirst("bytes".count)), let value = data?.stringValue {
                ...
                if value.starts(with: "0x"), let hex = Data(value.hexToBytes) ?? nil {    return try ABIValue(hex, type: .bytes(length))
                } else {
...
private extension Data {
    ...
    var sha3: Data {
        //Crypto.sha3(self)
        Data(SHA3(variant: .keccak256).calculate(for: [UInt8](self)))
    }

// https://github.com/horizontalsystems/EvmKit.Swift/blob/master/Sources/EvmKit/Helper/RLPElement.swift
import Foundation
import BigInt
...
class RLPElement {
    ...
    func intValue() throws -> Int {
        ...
        guard let uInt = UInt(dataValue.hexString, radix: 16) else {
            throw RLP.DecodeError.invalidIntValue
        }
    ...
    func bigIntValue() throws -> BigUInt {
        ...
        guard let bigInt = BigUInt(dataValue.hexString, radix: 16) else {
            throw RLP.DecodeError.invalidBigIntValue
        }
